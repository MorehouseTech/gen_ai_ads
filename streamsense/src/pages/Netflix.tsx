                            ...
                            {row.map((video) => (
                                <Grid item xs={12} sm={6} md={4} key={video.id}>
                                    <Box sx={{ position: 'relative', borderRadius: 1, overflow: 'hidden', '&:hover': { transform: 'scale(1.05)', '& .play-button': { opacity: 1 } }, transition: 'transform 0.3s' }}>
                                        <img src={video.image} alt={video.title} style={{ width: '100%', height: 160, objectFit: 'cover' }} />
                                        ...
                                        {(rowIndex === 1 || video.id % 3 === 0) && (
                                            <Box sx={{ position: 'absolute', bottom: 5, right: 5, bgcolor: rowIndex === 1 ? 'transparent' : 'red', color: 'white', px: 1, py: 0.5, borderRadius: 0.5, fontSize: '0.75rem' }}>
                                                {rowIndex === 1 ? 'Watch Now' : 'New Episode'}
                                            </Box>
                                        )}
                                    </Box>
